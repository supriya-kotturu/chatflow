...
func (s *Server) ChatRoomHandler(w http.ResponseWriter, r *http.Request) {
	...
	ctx := client.Room.Ctx
	s.broadcastAndPublish(ctx, client, resp, clientIp)
	s.RecordSuccess()
...
// writeTimeout is the maximum time allowed for a single WebSocket write.
// If exceeded, the client is considered dead and the connection is closed.
// This prevents a slow/stalled home client from filling its Send channel,
// which would cause LEAVE echoes to be dropped and the test to hang forever.
	...
	for {
		...
		case resp, ok := <-client.Send:
			...
			if err := client.Conn.WriteJSON(resp); err != nil {
				...
				client.Conn.Close() // unblock ReadJSON in ChatRoomHandler
				return
			}
...
// The broadcast send is non-blocking; the RabbitMQ publish blocks when the
// publish channel is full, applying back-pressure to the WebSocket read loop.
func (s *Server) broadcastAndPublish(ctx context.Context, client *Client, resp *models.Response, clientIp string) {
	select {
	case client.Room.Broadcast <- roomEvent{resp: resp}:
	default:
		client.Room.DroppedBroadcast.Add(1)
	...
	if s.Rabbit != nil {
		if err := s.Rabbit.Publish(ctx, client.Room.ID, qMessage); err != nil {
...
func (s *Server) scheduleRemoveUser(client *Client) {
	...
	default:
		...
		// Close Conn so the client-side read loop unblocks (it won't get the LEAVE
		// echo since the channel was full, but at least it exits cleanly).
		client.Conn.Close()
